...
/**@brief Compute number of bytes required to hold the application timer data structures. 计算保存应用程序计数器数据结构所需的字节数
 *
 * @param[in]  OP_QUEUE_SIZE   Size of queues holding timer operations that are pending execution. //等待执行的计时器操作队列大小
 *                             Note that due to the queue implementation, this size must be one more //注意，由于队列的实现，这个大小必须比实际的大小多一个。
 *                             than the size that is actually needed.
 *
 * @return     Required application timer buffer size (in bytes). //所需的应用程序计时器缓冲区大小。
 */
...
/**@brief Convert milliseconds to timer ticks. //将毫秒转换为计时器。
 *
...
/**@brief Initialize the application timer module. //初始化应用程序计时器模块
 *
 ...
 * @param[in]  SCHEDULER_FUNC   Pointer to scheduler event handler //指向调度程序事件处理程序的指针
 *
...
/**@brief Function for initializing the timer module. //初始化定时器模块
 *
 ...
 *
 * @param[in]  p_timer_id        Pointer to timer identifier. //计时器指针标识符
 * @param[in]  mode              Timer mode. //定时器模式
 * @param[in]  timeout_handler   Function to be executed when the timer expires. //当定时器到期时执行的函数
 *
 * @retval     NRF_SUCCESS               If the timer was successfully created. //当计时器成功创建时。
