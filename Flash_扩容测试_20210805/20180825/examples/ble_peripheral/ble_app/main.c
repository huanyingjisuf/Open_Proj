...
//#define FC_OP_Key  1   //防拆改 开门 宏定义 屏蔽后 防拆变为开门
...
void Marnual_Add_Run()
{
	...
	if ((Start_Manual_Add_Card>0)&&(cardStatus == WAITREMOVE)&&(read_TypeA_Data[0]==0x01)&&(Type_AB_UID[0]==0x01)&&(BLE_LINK==1))  //当读到有效 A 卡 且有自助发卡指令 蓝牙连接时
	{
...
uint8_t Err_PassWord_CL;  //定时清空错误密码次数
...
//#define  History_Time_Act_PassWord_Run  1				//开启历史动态密码屏蔽功能
