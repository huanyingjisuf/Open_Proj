...
/**@brief Function for initializing the BLE Button Module. //初始化BLE按键模块的函数
 *
 * Before calling this function, the BSP module must be initialized with buttons. //在调用这个函数之前，BSP模块必须用按钮来初始化
 *
 * @param[out] error_handler      Error handler to call in case of internal errors in BLE Button //在BLE按钮模块中出现内部错误的错误处理程序
 *                                Module.
