 ...
 *
 * @brief BSP module.
 * @details This module provides a layer of abstraction from the board. //这个模块提供了一个来自板的抽象层
...
typedef enum
{
    BSP_EVENT_NOTHING = 0,                  /**< Assign this event to an action to prevent the action from generating an event (disable the action). */ //将事件分配给一个动作，以防止动作产生(禁用动作)
    BSP_EVENT_DEFAULT,                      /**< Assign this event to an action to assign the default event to the action. */ //把这个事件分配给一个动作，将默认事件分配给动作。
    BSP_EVENT_CLEAR_BONDING_DATA,           /**< Persistent bonding data should be erased. */ // 持续的绑定数据应该被清除。
    BSP_EVENT_CLEAR_ALERT,                  /**< An alert should be cleared. */ //清除一个警告
    BSP_EVENT_DISCONNECT,                   /**< A link should be disconnected. */ //断开一个连接
    BSP_EVENT_ADVERTISING_START,            /**< The device should start advertising. */ //这个设备应该开始广播。
    BSP_EVENT_ADVERTISING_STOP,             /**< The device should stop advertising. */ //这个设备应该停止广播。
    BSP_EVENT_WHITELIST_OFF,                /**< The device should remove its advertising whitelist. */ // 这个设备应该删除其广播白名单
    BSP_EVENT_BOND,                         /**< The device should bond to the currently connected peer. */ //这个设备应该与当前连接的对等点绑定。
    BSP_EVENT_RESET,                        /**< The device should reset. */ //这个设备应该重置。
    BSP_EVENT_SLEEP,                        /**< The device should enter sleep mode. */ //该设备应该进入睡眠模式。
    BSP_EVENT_WAKEUP,                       /**< The device should wake up from sleep mode. */ //该设备应该从睡眠模式中醒来
    BSP_EVENT_DFU,                          /**< The device should enter DFU mode. */ //该设备应该进入DFU模式。
    BSP_EVENT_KEY_0,                        /**< Default event of the push action of BSP_BUTTON_0 (only if this button is present). */ //BSP_BUTTON_0 推送操作的默认事件 (只有当这个按钮出现时。)
...
typedef struct
{
    bsp_event_t push_event;      /**< The event to fire on regular button press. */ //按常规按钮按下的事件。
    bsp_event_t long_push_event; /**< The event to fire on long button press. */ //用长按钮按下的事件
    bsp_event_t release_event;   /**< The event to fire on button release. */ //触发按钮释放的事件。
...
/**@brief       Function for initializing BSP. //函数初始化BSP
 *
...
/**@brief       Function for assigning a specific event to a button. //将特定时间分配给一个按钮的函数
 *
 * @details     This function allows redefinition of standard events assigned to buttons. //这个函数允许重新分配定义分配给按钮的标准事件
 *              To unassign events, provide the event @ ref BSP_EVENT_NOTHING. 要取消指定事件，提供事件 BSP_EVENT_HOTHING.
 *
 * @param[in]   button                   Button ID to be redefined. //重新定义按钮ID
 * @param[in]   action                   Button action to assign event to. // 将事件分配给按键操作。
 * @param[in]   event                    Event to be assigned to button. // //事件分配给按钮
 *
 * @retval      NRF_SUCCESS              If the event was successfully assigned to button. //如果事件被成功地分配给按钮
 * @retval      NRF_ERROR_INVALID_PARAM  If the button ID or button action was invalid. //如果按钮ID或按钮操作无效。
 */
 //button是数组 m_events_list的下标，可见如果在次初始化的时候可能把event给改变。
...
uint32_t Bsp_Set(void);   //在运行中重置 BSP
