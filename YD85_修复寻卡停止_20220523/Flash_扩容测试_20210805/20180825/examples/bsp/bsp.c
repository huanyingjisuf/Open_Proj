...
#ifndef BSP_SIMPLE
/*
app_buttons 参数，由此可见，需要配置哪个端口，低电平(false)还是高电平(ture)有效，管脚模式(上下拉等等)对应的函数
*/
static const app_button_cfg_t app_buttons[BUTTONS_NUMBER] =
{
    ...
    #endif // BUTTON_0
	#endif
    #ifdef BSP_BUTTON_1  //  外部复位 钥匙 中断
    {BSP_BUTTON_1, false, BUTTON_PULL, bsp_button_event_handler},
    #endif // BUTTON_1

    #ifdef BSP_BUTTON_2  //  外部刷卡中断
    {BSP_BUTTON_2,true, BUTTON_DOWN, bsp_button_event_handler},
    #endif // BUTTON_2
		
    #ifdef BSP_BUTTON_3    
    {BSP_BUTTON_3, true, BUTTON_DOWN, bsp_button_event_handler},
    #endif // BUTTON_3
 ...
 *
 * @param[in]   pin_no          The pin number of the button pressed. //按下按钮的号码
 * @param[in]   button_action   Action button. //操作蓝牙
 */
static void bsp_button_event_handler(uint8_t pin_no, uint8_t button_action)
{
...
//    static uint8_t     current_long_push_pin_no;              /**< Pin number of a currently pushed button, that could become a long push if held long enough. */ 
	//一个当前按下的按钮的pin码，如果保持足够长的时间，它可能会成为一个长时间的推送。
 //   static bsp_event_t release_event_at_push[BUTTONS_NUMBER]; /**< Array of what the release event of each button was last time it was pushed, so that no release event is sent if the event was bound after the push of the button. */
	//每个按钮的发布时间的数组是在上次被推送的时候，所以如果事件在按钮的推动下被绑定，那么就不会发生任何发布事件。
	
	bsp_event_handler_O(); //按键中断全部进入这个函数
...
#if LEDS_NUMBER > 0 && !(defined BSP_SIMPLE)
    m_app_ticks_per_100ms = ticks_per_100ms; //消抖动事件赋值
...
#if (BUTTONS_NUMBER > 0) && !(defined BSP_SIMPLE)
//    m_registered_callback = callback; //回调函数赋值及 bsp_event_handler

    // BSP will support buttons and generate events //判断是否按键事件
    if (type & BSP_INIT_BUTTONS)
    {
        uint32_t num;
	// BUTTONS_NUMBER 是定义了几个中断，中断类型回调函数初始化
...
/*		 //初始化LED事件
    ...
    // timers module must be already initialized! //定时器模块必须已经初始化
    if (err_code == NRF_SUCCESS)
    {
        err_code =
...
#ifndef BSP_SIMPLE
/**@brief Assign specific event to button. //将特定事件分配给按钮。
 */
uint32_t bsp_event_to_button_action_assign(uint32_t button, bsp_button_action_t action, bsp_event_t event)
{
    uint32_t err_code = NRF_SUCCESS;

#if BUTTONS_NUMBER > 0  //注册过按键时
	
	
    if (button < BUTTONS_NUMBER) //判断是否定义了按键。
    {
        if (event == BSP_EVENT_DEFAULT) //判断事件是否为 BSP_EVENT_DEFAULT事件
        { //找到定义的哪个按键的事件。
            // Setting default action: BSP_EVENT_KEY_x for PUSH actions, BSP_EVENT_NOTHING for RELEASE and LONG_PUSH actions. 
					//设置默认的行动：BSP_EVENT_KEY_x 为推动的行为，BSP_EVENT_NOTHING用于发布(RELEASE) 和 长推送 (LONG_PUSH) 动作。
            event = (action == BSP_BUTTON_ACTION_PUSH) ? (bsp_event_t)(BSP_EVENT_KEY_0 + button) : BSP_EVENT_NOTHING;
        }
        switch (action)
        {
            case BSP_BUTTON_ACTION_PUSH:
//                m_events_list[button].push_event = event; //给 m_events_list[button].push_event 赋值
                break;
            case BSP_BUTTON_ACTION_LONG_PUSH:
