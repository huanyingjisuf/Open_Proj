...
extern uint8_t Lock_Open_EN;    //播放开关

const uint8_t OP_ALL_Ceng[11]={"00099123456"};
...
uint8_t Data_Card_Run()
{
	...
	uint8_t Dong_Num;   //设备楼栋号
	
	uint8_t Card_Dong_Num;  //卡片楼栋号
	uint64_t Ck_Data;  //加密计算结果
	uint64_t Card_Ck_Data;  //卡内加密计算结果
	uint64_t anth_index;  //卡内序列号
	uint8_t i;
	uint64_t card_mng_code;  //判断梯控卡数据
	...
	if((read_TypeA_Data[17]==0x55)&&(read_TypeA_Data[18]==0xAA))
	{
		...
		if(read_TypeA_Data[19]==0xE8)    //开/关 门未关好语音
		{
			Ev_Cat_Dat(0x50,read_TypeA_Data[20]);					//写入初始数据
	...
	Dong_Num =Devic_Num[52];
	
	#ifdef uart_start

	SEGGER_RTT_printf(0,"Ceng_Num:%x",Floor_Num);
	SEGGER_RTT_printf(0,"\r\n");

	SEGGER_RTT_printf(0,"Dong_Num:%x",Dong_Num);
	SEGGER_RTT_printf(0,"\r\n");
	...
	card_mng_code = read_TypeA_Data[1];
	card_mng_code<<=8;
	
	card_mng_code |= read_TypeA_Data[2];
	card_mng_code<<=8;
	
	card_mng_code |= read_TypeA_Data[3];
	card_mng_code<<=8;
	
	card_mng_code |= read_TypeA_Data[4];
	
	//梯控卡过期时间
	outtime = read_TypeA_Data[11];
	outtime<<=8;
	
	outtime |= read_TypeA_Data[12];
	outtime<<=8;
	
	outtime |= read_TypeA_Data[13];
	outtime<<=8;
	
	outtime |= read_TypeA_Data[14];
	...
	SEGGER_RTT_printf(0,"Unix_T:%x\r\n",Unix_T);
	SEGGER_RTT_printf(0,"outtime:%x\r\n",outtime);
...
//		
	...
	else if(read_TypeA_Data[17]==0x02)  //当数据是管理卡时
	{
		...
		Card_Dong_Num = read_TypeA_Data[35];  //获取卡片的楼栋号
		...
		if((TIME_STATE==true)&&((Card_Floor_Num==Floor_Num)||(Card_Floor_Num==0))&&((Card_Dong_Num==Dong_Num)||(Card_Dong_Num==0))&&((intime<Unix_T)&&(outtime>Unix_T))&&
