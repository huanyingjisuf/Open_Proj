...
uint32_t SPI_FLASH_CS =20;
uint32_t SPI_FLASH_CLK =18;
uint32_t SPI_FLASH_MOSI =17;
uint32_t SPI_FLASH_MISO = 19;

#define SUCCESS                0x01
#define FAIL                0x00
#define FLASH_SPI		SPI0  //声明SPI0接口用于FLASH通讯

uint8_t SPI0_Start =0; //确认SPI0 是否被启用
...
static uint32_t spi_master_init(void)
{
	...
	nrf_gpio_pins_set(SPI_FLASH_CS);
	
	NRF_SPI0->ENABLE = 0;
	NRF_SPI0->CONFIG =0;
	NRF_SPI0->INTENCLR =1;  //清除中断标志
	NRF_SPI0->INTENSET =0;
	
	
	NRF_SPI0->CONFIG = (0<<0)|(0<<1)|(0<<2);
	NRF_SPI0->FREQUENCY = 0X80000000; //速率
	NRF_SPI0->PSELSCK = SPI_FLASH_CLK;
	NRF_SPI0->PSELMOSI = SPI_FLASH_MOSI;
	NRF_SPI0->PSELMISO = SPI_FLASH_MISO;
	NRF_SPI0->ENABLE = 1;
...
uint8_t SpiFlash_ReadSR(void)  //读取状态寄存器
{
    uint8_t retValue = 0;
    SPIFlash_Enable_CS;
...
/*****************************************************************************
** 文件名称：void SpiFlash_PowerDown(void)
** 功    能：
** 修改日志：
******************************************************************************/

void SpiFlash_PowerDown(void) //进入掉电模式
{
//    SPIFlash_Enable_CS;
//    SpiFlash_WriteOneByte(SPIFlash_PowerDown_CMD);
//    SPIFlash_Disable_CS;

//		#ifdef uart_start
//		
//		SEGGER_RTT_printf(0,"Spi_Flash_PowerDow!\r\n");
//		
//		#endif
}

/*****************************************************************************
** 文件名称：void SpiFlash_PowerUp(void)
** 功    能：
** 修改日志：
******************************************************************************/

void SpiFlash_PowerUp(void) //上电模式
{
    SPIFlash_Enable_CS;
    SpiFlash_WriteOneByte(SPIFlash_PowerUp_CMD);
    SPIFlash_Disable_CS;
}

/*****************************************************************************
** 文件名称：void SpiFlash_EnableReset(void)
** 功    能：
** 修改日志：
******************************************************************************/

void SPIFlash_EnableReset(void) //开启复位指令
{
    SPIFlash_Enable_CS;
	 SpiFlash_WriteOneByte(SPIFlash_Enable_Reset_CMD);

    SPIFlash_Disable_CS;
}

/*****************************************************************************
** 文件名称：void SpiFlash_Reset(void)
** 功    能：
** 修改日志：
******************************************************************************/

void SPIFlash_Reset(void) //复位指令
{
    SPIFlash_Enable_CS;
    SpiFlash_WriteOneByte(SPIFlash_Reset_CMD);
    SPIFlash_Disable_CS;
}

/*****************************************************************************
** 文件名称：void SPIFlash_CMD_init(void)
** 功    能：
** 修改日志：
******************************************************************************/

void SPIFlash_CMD_init(void) //初始化寄存器指令
{
	SPIFlash_EnableReset();
	
	nrf_delay_us(50); //复位间隔
...
void SPI_Flash_Start(void)
{
		...
		nrf_gpio_cfg_input(SPI_FLASH_MISO,NRF_GPIO_PIN_NOPULL);
		nrf_delay_ms(1);
		spi_master_init(); //开启硬件SPI
		SPIFlash_CMD_init();
		SPIFlash_CMD_init(); //初始化寄存器
		
		#ifdef uart_start
		SEGGER_RTT_printf(0,"SPI0_Start!\r\n");
		#endif
		SPI0_Start =1;  //表示 SPI0 已开启
		nrf_delay_ms(10);
...
void Spi_Flash_Sleep(void)  //进入待机，关闭SPI Flash 接口 
{
		if(SPI0_Start==1)
		{
			...
			CL_SKY1311_IRQ();//在SPI待机前，清空 SKY1311的中断
		
			
			
			nrf_gpio_cfg_input(SPI_FLASH_CLK,NRF_GPIO_PIN_PULLUP);
			nrf_gpio_cfg_input(SPI_FLASH_MOSI,NRF_GPIO_PIN_PULLUP);
			nrf_gpio_cfg_input(SPI_FLASH_MISO,NRF_GPIO_PIN_PULLUP);
		
			NRF_SPI0->ENABLE = 0;
			SPI0_Start =0;  //表示 SPI0 已关闭
		
			#ifdef uart_start
		
			SEGGER_RTT_printf(0,"SPI0_Stop!\r\n");
