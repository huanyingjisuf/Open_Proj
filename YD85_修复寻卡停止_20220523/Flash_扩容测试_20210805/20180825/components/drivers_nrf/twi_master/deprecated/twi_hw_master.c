...
#define MAX_TIMEOUT_LOOPS (20000UL) /**< MAX while loops to wait for RXD/TXD event通讯最长等待时间 */
...
static bool twi_master_write(uint8_t * data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    NRF_TWI1->TXD           = *data++;
    NRF_TWI1->TASKS_STARTTX = 1;  //开始发送任务

    /** @snippet [TWI HW master write] */ // TWI总线写操作
    while (true)
    {
        while (NRF_TWI1->EVENTS_TXDSENT == 0 && NRF_TWI1->EVENTS_ERROR == 0 && (--timeout)) //等待发送完成
        {
            // Do nothing.
        }

        if (timeout == 0 || NRF_TWI1->EVENTS_ERROR != 0) //当发生错误或超时时 进入错误处理 发送通知异常 重置外部总线
        {
            ...
            return false;  //结束本次数据传送
        }
        NRF_TWI1->EVENTS_TXDSENT = 0; 
        if (--data_length == 0)  //数据是否发送完成
        {
            break;   //数据发送完成后跳出循环
        }

        NRF_TWI1->TXD = *data++; //还有数据则继续传送
    }
    /** @snippet [TWI HW master write] *///数据发送完成后停止TWI数据传送
    ...
    if (issue_stop_condition)
    {
        ...
        while(NRF_TWI1->EVENTS_STOPPED == 0)  //事件是否·真的停止
        {
            // Do nothing.
        }
    }
    return true; //返回数据发送成功
}
 ...
 */
bool twi_master_init(void) //初始化总线
{
...
/** @brief  Function for transfer by twi_master. //I2C主机写模式
 */
...
#define i2c_device_address 0x5b
//在制定处读取制定字节的数据，源码：
